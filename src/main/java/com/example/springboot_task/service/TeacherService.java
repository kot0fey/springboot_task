...
public class TeacherService {
    ...
    public ResponseDto getAllTeachers(int offset, int limit) {
        Pageable pageable = new OffsetBasedPageRequest(offset, limit);
        List<TeacherDto> teacherDtoList = teacherRepository
                .findAll(pageable)
                .stream()
                .map(TeacherMapper::mapToTeacherDto)
                .toList();
        long total = teacherRepository
                            .count();
        ResponseDto<TeacherDto> responseDto = new ResponseDto<>(teacherDtoList, total);
        return responseDto;
    }
